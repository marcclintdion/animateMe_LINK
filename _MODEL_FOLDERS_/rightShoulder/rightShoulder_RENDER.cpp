runTripleQuat(6); //__MAYBE_THIS_CAN_BE_MOVED_TO_THE_SHADOW_PASS_SO_ITS_ONLY_CALCULATED_ONCE :) THIS_WILL_REQUIRE_THAT_PARTS_HAVE_UNIQUE_MATRICES_SO_THEY_ARE_INSULATED_FROM_SUBSEQUENT_TRANSFORMS 
setupTransforms_MainColor();
    ...
    LoadIdentity(rightShoulder_MATRIX); 
    AssignMatrix(rightShoulder_MATRIX, torso_MATRIX);  
    LoadIdentity(quaternionMatrix); 
    LoadIdentity(tempMatrix_XY); 
    LoadIdentity(tempMatrix_Y); 
    LoadIdentity(tempMatrix_Z); 
    //--------------------------------------------------------
    Translate(rightShoulder_MATRIX, rightShoulder_POSITION[0] + moveSet[0],  
    ...
    MultiplyMatrix(modelWorldMatrix, rightShoulder_MATRIX ,  quaternionMatrix); 
    AssignMatrix(rightShoulder_MATRIX, modelWorldMatrix); 
    Translate(rightShoulder_MATRIX, -1.0 * (rightShoulder_POSITION[0] + moveSet[0]),  
                                   -1.0 * (  rightShoulder_POSITION[1] + moveSet[1]), 
                                   -1.0 * ( rightShoulder_POSITION[2] + moveSet[2])); 
//===================================================   
